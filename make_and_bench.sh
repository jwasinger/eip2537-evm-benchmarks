...
bench_stats=$($GETH_EVM --codefile $bench_tmp_file --input $input --prestate ./genesis.json --bench run 2>&1 | python3 calc_stats.py)
noop_stats=$($GETH_EVM --codefile $noop_tmp_file --input $input --prestate ./genesis.json --bench run 2>&1 | python3 calc_stats.py)
